...
void menu(uint8_t*** &image, int width, int height)
{
	...
	{
	case(0): break;
	case(1): grey_image1(image, width, height); grey_pixel_chart(image, width, height); break;
	case(2): grey_image2(image, width, height); grey_pixel_chart(image, width, height); break;
	case(3): grey_image1(image, width, height); grey_equalize(image, width, height); grey_pixel_chart(image, width, height); break;
	case(4): grey_image1(image, width, height); grey_sharpening(image, width, height); grey_image1(image, width, height); grey_smooth(image, width, height); break;
	case(5): RGB_pixel_chart(image, width, height); break;
	case(6): RGB_equalize(image, width, height); RGB_pixel_chart(image, width, height); break;
	case(7): RGB_sharpening(image, width, height); RGB_smooth(image, width, height); break;
	}
}
int main()
{
