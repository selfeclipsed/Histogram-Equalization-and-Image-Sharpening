...
void pixel_counting(uint8_t*** image, int width, int height, int pcnt[])
{
	for (int p = 0; p <= 255; p++)
	{
...
void pixel_chart(uint8_t*** image, int width, int height ,int pcnt[])
{
	ofstream dataFile;
...
void equalize(uint8_t*** image, int width, int height, int pcnt[])
{
	int psum = 0;
	for (int i = 0; i < height; ++i)
	{
...
void sharpening(uint8_t*** image, int width, int height)
{
...
void smooth(uint8_t*** image, int width, int height)
{
...
int main()
{
	...
	uint8_t*** image;
	int i, j;
	char readPath[] = "C:/opencv/pictures/1.bmp";
	readBmp(readPath, image, bmpWidth, bmpHeight);
	cout << "width=" << bmpWidth << " height=" << bmpHeight << endl;
	grey_image2(image, bmpWidth, bmpHeight);
	//pixel_counting(image, bmpWidth, bmpHeight, pcnt);
	//equalize(image, bmpWidth, bmpHeight,pcnt);
	//pixel_counting(image, bmpWidth, bmpHeight, pcnt);
	//pixel_chart(image, bmpWidth, bmpHeight,pcnt);
	sharpening(image, bmpWidth, bmpHeight);
	//pixel_counting(image, bmpWidth, bmpHeight, pcnt);
	//equalize(image, bmpWidth, bmpHeight,pcnt);
	smooth(image, bmpWidth, bmpHeight);
	pixel_counting(image, bmpWidth, bmpHeight, pcnt);
